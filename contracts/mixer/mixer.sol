...
contract RingTokenMixer {
    //Debug Code
    address public owner;
    function RingTokenMixer(IERC20 _tokenContract) public {
        //Debug Code
        owner = msg.sender;
    ...
    //Deposit tokens as RingMixer tokens to the specified RingMixer address
    function Deposit(address destination, uint numToken)
        payable public returns (bool success)
    {
        ...
        require(numToken > 0);
        require(tokenContract.transferFrom(msg.sender, address(this), numToken));

        //Add tokens to balance corrosponding to the amount of tokens
        token_balance[destination] = numToken;
        success = true;
    }
        ...
        public returns (bool success)
    {
        ...
        if (success) {
            ...
            for (i = 0; i < destination.length; i++) {
                tokenContract.transfer(destination[i], value[i]);
            }
        }
    }
